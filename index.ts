...
(async (): Promise<void> => {
  const browser: Browser = await puppeteer.launch({ timeout: 100000 });
  ...
  const width: number = iPhone13.viewport.width;
  const height: number = iPhone13.viewport.height;
  ...
  const p1 = await page.screenshot({ clip: { x: 0, y: 0, width, height } });
  await waitForTimeout(500);
  const p2 = await page.screenshot({ clip: { x: 0, y: 0, width, height } });
  await waitForTimeout(500);
  const p3 = await page.screenshot({ clip: { x: 0, y: 0, width, height } });
  await waitForTimeout(500);
  const p4 = await page.screenshot({ clip: { x: 0, y: 0, width, height } });
  await waitForTimeout(500);
  const p5 = await page.screenshot({ clip: { x: 0, y: 0, width, height } });
  await waitForTimeout(500);
  const p6 = await page.screenshot({ clip: { x: 0, y: 0, width, height } });
  await waitForTimeout(500);
  const p7 = await page.screenshot({ clip: { x: 0, y: 0, width, height } });
  await waitForTimeout(500);
